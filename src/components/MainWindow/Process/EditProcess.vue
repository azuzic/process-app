<template>
    <div class="bg-main_bg px-4 flex flex-col pb-3">
        ...
        <div class="flex flex-col h-full">
            ...
            <input @input="$store.state.processUpdated ? $store.dispatch('checkProcessUpdate') : ''" v-model="$store.state.process.name" class="vue-input2 text-sm" placeholder="Enter process name ..." type="text">
            
            <!--PROCESS DETAILS-->
            <b class="text-lg pl-1 my-1">Details</b>
            <textarea @input="$store.state.processUpdated ? $store.dispatch('checkProcessUpdate') : ''" v-model="$store.state.process.details" class="vue-input2 text-sm" placeholder="Enter details ..." type="text" rows="10"></textarea>
